...
public final class Preconditions {
   ...
   */
  @PreferKotlinApi(replacement = "require(expression)") // PUBLIC:strip_line
   ...
   */
  // PUBLIC:begin_strip
  @PreferKotlinApi(replacement = "require(expression) { errorMessage ?: \"null\"}")
  // PUBLIC:end_strip
   ...
   * @throws IllegalArgumentException if {@code expression} is false
   */
  // Exposing this could help Kotlin but risk NoSuchMethodError: b/322383297#comment13

  // TODO: b/322383297 - @PreferKotlinApi
  // (replacement = "require(expression) { errorMessage ?: \"null\"}")
  public static <ImpossibleT extends String & Impossible> void checkArgument(
      boolean expression, @Nullable ImpossibleT errorMessage) {
    if (!expression) {
      // Convert a null message to the string "null"
      throw new IllegalArgumentException(Platform.stringValueOf(errorMessage));
    }
  }
   ...
   */
  @PreferKotlinApi(replacement = "check(expression)") // PUBLIC:strip_line
   ...
   */
  // PUBLIC:begin_strip
  @PreferKotlinApi(replacement = "check(expression) { errorMessage ?: \"null\"}")
  // PUBLIC:end_strip
  ...
  // TODO: b/322383297 - @PreferKotlinApi
  // (replacement = "check(expression) { errorMessage ?: \"null\"}")
  public static <ImpossibleT extends String & Impossible> void checkState(
      boolean expression, @Nullable ImpossibleT errorMessage) {
    if (!expression) {
      // Convert a null message to the string "null"
      throw new IllegalStateException(Platform.stringValueOf(errorMessage));
  ...
  // PUBLIC:begin_strip
  @PreferKotlinApi(
      replacement = "requireNotNull(reference)",
      comment =
          "Prefer Kotlin's `requireNotNull(Any?)`, but note that that function throws"
              + " IllegalArgumentException instead of NullPointerException.")
  // PUBLIC:end_strip
  ...
  // PUBLIC:begin_strip
  @PreferKotlinApi(
      replacement = "requireNotNull(reference) { errorMessage ?: \"null\" }",
      comment =
          "Prefer Kotlin's `requireNotNull(Any?, () -> Any)`, but note that that function throws"
              + " IllegalArgumentException instead of NullPointerException.")
  // PUBLIC:end_strip
  @CanIgnoreReturnValue
  ...
  //            + " IllegalArgumentException instead of NullPointerException.")
  @CanIgnoreReturnValue
  public static <T, ImpossibleT extends String & Impossible> T checkNotNull(
      T reference, @Nullable ImpossibleT errorMessage) {
    if (reference == null) {
      // Convert a null message to the string "null"
      throw new NullPointerException(Platform.stringValueOf(errorMessage));
    }
